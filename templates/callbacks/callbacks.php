...
//========== Register User Verifcation =========

add_action( 'user_register', 'registration_verification', 10, 2 );
function registration_verification( $user_id ) {
    ...
    $click="Click Here Verify Your Account";

    $url = get_site_url(). '/login/?act=' .base64_encode( serialize($string));
    $html = 'Please click the following links <br/><br/> <a href="'.$url.'">'.$click.'</a>';

    //$html = 'Hi "'.$username.'", Thank you for registering at FREESALE!  Please check your emails to verify your account. '
    wp_mail( $user_info->user_email, __('Free Sales Email Varifiction','text-domain') , $html);
}
...
function verify_user_code(){
    if(isset($_GET['act'])){
        ...
        if($code == $data['code']){
            ...
            //echo "<p>' . __('<strong>Success:</strong> Your account has been activated! ', 'text-domain') . '</p>";
...
function bs_login_callback_callback() {
    ...
    if ($exists || $bs_username) {
        ...
        $useractive=get_user_meta($infouser->ID,'account_activated',true);
...
function bs_update_profile_callback() {
    ...
    $bs_fullname     = $_REQUEST['bs_fullname'];

    $bs_phonenumber  = $_REQUEST['bs_phonenumber'];
    ...
    if (!is_wp_error($user_id)) {
        ...
        update_user_meta($user_id, 'fullname', $bs_fullname);
        update_user_meta($user_id, 'phonenumber', $bs_phonenumber);
